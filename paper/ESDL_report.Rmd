...
abstract: |
  ...
  anthropogenic pressures and climate change. Yet it is unclear where the risks are higher or where in the world ecosystems are more vulnerable. Here I measure resilience of primary productivity proxies for marine and terrestrial ecosystems globally. Up to 29% of global terrestrial ecosystem, and 24% marine ones, show symptoms of resilience loss. These symptoms are shown in all biomes, but by large Arctic tundra and boreal forest are the most affected, as well as the Indian Ocean and Eastern Pacific. Although the results are likely an underestimation, they enable the identification of risk areas as well as the potential synchrony of some transitions, helping prioritize areas for management interventions and conservation.
...
basin [@Titus:2020hb]. Both techniques pick up resilience loss by measuring
changes in the higher moments of the time series distribution by detecting
...
how easily it finds modes to deal with disturbances, that has found applications
in the diagnosis of cardiac disorders [@West:2017wo; @West:2010db] and
engineering [@Pavithran:2021jc]. Exit time has also being proposed as a
...
to generic early warnings which could help circumbent some of their limitations
by calculating changes on the leading eignvalue in time series, if $\lambda >1$
the timeseries signals a loss of stability and resilience [@Ives:2012df;
@Taranu:2018dg; @Carpenter:2013cr]. On the empirical realm, recent studies
has pointed to remote sensing products [@Verbesselt:2016hn] and climate
...
productivity. It applies the traditional early warning signals based on critical
...
series is first pre-processed to remove confoudning factors such as seasonality
...
series were first-differenced and passed a root test that guarantees
...
against. In the pixel described in Fig \ref{fig:sm-onepxl} the observed $\Delta$
...
interpreted here as symptoms of resilience loss, are detected in a number of
different arrangments being them most common agreement between kurtosis and
skewness (Fig \ref{fig:sm-gpp},\ref{fig:sm-ter}, \ref{fig:sm-mar}).
...
Caribbean basin in the Tropical Atlantic, the Tropical Pacific and south east
Madagascar.
...
\begin{figure*}[ht]
\centering
\includegraphics[width = 7in, height = 7in]{figS_permutations}
\caption{\textbf{Permutations tests over time} A sample of 500 pixels where resilience loss was detected by at least three indicators is compared against 5000 permutations of the same time series (10 permutations each). A Wilconxon two sided test confirms differences in mean between the real and permuted time series (p << 0.05 for all comparisons).}
\label{fig:perms}
\end{figure*}
```
An alternative interpretation for coherence of signals is spatial and temporal
autocorrelations, or that they are merely responses to shocks on environmental
...
[@ives2021statistical]. Issues of spatial autocorrelation are dealt (below) by
...
dissapears, meaning it has e.g. a signficantly lower autocorrelation and
standard deviation on the absolute value scale (Fig \ref{fig:perms}). A Wilcoxon
two sided test comparing the distributions of permutations agains observed data
suggest that the means are indeed different (p \<\< 0.05 for all comparisons).
Thus the results are not merely noise induced by outliers (e.g. shocks) because
the same time series with the same outliers but different ordering does not rise
the early warnings to the same extend that the observed data. On the contrary,
for some statistics like autocorrelation and standard devitation, their delta
values go back to what is expected to be normal (the global median).

An additional robustness test is comparing the generic indicators agains
model-based early warnings. The latter are autoregressive dynamic linear models
...
@Taranu:2018dg; @Carpenter:2013cr]. For each dataset a sample of a hundred
...
\begin{figure*}[ht]
\centering
\includegraphics[width = 7.3in, height = 3in]{lambda_plots}
\caption{\textbf{Model-based indicators} A sample of 100 pixels where resilience loss was detected by at least three indicators is compared against 100 pixels where no signals were detected. The leading eignevalue of the system is approximated with a dynamic linear autoregressive model}
\label{fig:lambda}
\end{figure*}
```
If resilience is approximated as the size of the basin of attraction, critical
slowing down picks up reductions in depth while critical speeding up is sensible
to reductions in width. However, these theories when applied to ecological
problems are typically dealt as low dimensional models with perhaps one
controlling factor, one driver. Recent experimental evidence suggest that when
...
least two proxies of resilience loss was used as response variable. Explanatory
...
rainforests [@Duffy:2021br]. These are biomes where strongest signals of
...
dryland area can cross these thresholds by 2100 for the most drastic climate
...
oxygen minimum zones [@Breitburg:2018iz]. Our results present an additional
...
range of local cases previously reported [@Breitburg:2018iz]. Only regime
...
boreal forest and taiga [@Feng:2021gr]. The estimates here presented
...
Another limitation of this study is the lack of ground truth. The results
provide a spatial prediction of where ecosystems might be lossing their
resilience. But since critical transitions have not happened yet in many of
these places, it is not possible to contrast the models here presented with
...
productivity, but fail in supporting the findings of terrestrial ecosystem
respiration.

Model-based approaches such autoregressive time varying models can help
...
thus help distinguish what are optimal observables of resilience loss, an open
...
experiements. Because the ground truth is known, there was not concern whether
the magnitud of increase (decrease) in early warnings was big enough to actually
be considered a warning. Here we do not have ground truth as in experiments, but
we can compute the distribution of each indicator for the entire planet enabling
the comparison of what consitutes a big jump on an indicator as opposed to its
normal expected variability for each biome. The magnitud depends on the
indicator and data pre-processing choices, but the position of the pixel in the
distribution is relatively unnafected. Thus $\Delta$ is less sensitive to
...
towards the tails of the distribution (resilience loss) as opposed to a an
...
Despite of its limitations, this first-order approximation to resilience loss
...
land use planning and take into consideration the vulnerability of their
ecosystems. Companies for example, can take such risk into account when deciding
...
ecosystem service (e.g. a monoculture) in favour of multifunctional landscapes
