...
signals of resilience loss globally (Fig \ref{fig:gpp}, S1). However,
...
ecosystem respiration data sets (Fig S2, S3).
...
Southern Ocean (Fig \ref{fig:mar}, S4). The Easter Indo-Pacific
...
not artifacts of the data used (Fig S5, S6, S7). On the contrary, 
...
space (Fig S4) and time (Fig S7). There is high
agreement between kurtosis and skewness across all data sets (Figs
S2, S3, S4); they can signal the
...
values go back to what is expected to be normal (the global medium range, see i.e., Figs \ref{fig:delta}, or the insets in S2, S3, S4 for all variables).
...
\ref{fig:mar}, S1). The overall patterns here reported agree with
...
dimension, skewness and kurtosis (Fig. S2, S3).
